...
impl Bst {
    ...
    pub fn successor(&mut self, key: u32) -> Option<usize> {
        ...
        /* search for the lowest greater value for the current key */
        while curr != None {
    ...
    /* we don't check the property for simplicity 
     * this function is used only for swapping with successor */
    pub fn modify(&mut self, index: usize, key: u32) {
       self.v[index].key = key;
    }
...
impl Course {
    ...
    /* LIS on beauty array, sorted by difficulty */
    pub fn lis(&mut self) -> usize {
        ...
        /* lis algo based on successor: 
         * if key greater than bst max then we add it, 
         * if not and successor exists we swap it with key */
        for i in 1..b_by_d.len() {
            ...
            if key > max_key {
                ...
                if succ != None {
                    /* modify the successor preserve the bst property */
                    self.bst.modify(succ.unwrap(), key);
                }
            } 
        }
